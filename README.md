...
- `DaSiWa AutoLong GGUF Warm-up Stream Writer`
  - 面向 DaSiWa GGUF 原工作流，不需要原版 Wan2.2 模型或任何加速 LoRA。
  - 每段多生成一小段隐藏的预热区；下一段从预热区起点重新生成，并在重叠区后半段自动寻找差异最小的切换位置。
  - `warmup_frames = 16` 表示在 16 FPS 原始帧中保留 1 秒预热区；配合 2 倍 RIFE 时，生成 97 帧、最终每段写入 160 帧（5 秒）。
  - 只在 CPU 内存中暂存一小段 8-bit 重叠帧，不累计完整视频。
  - 智能切换不做透明叠化，因此不会产生双重人影；单帧续写不能传递真实运动状态，极端内容仍可能有轻微动作跳变。
  - 可选输入 `interp_model` 接入原工作流已经加载的 RIFE 模型后，会用 7 帧短 RIFE 桥替代直接硬切；它不做透明叠化，主要用于缓和动作顿挫，大幅姿势变化时可能出现几帧形变。
  - 最后一段完成时在同一个节点内显示播放器和下载按钮。
